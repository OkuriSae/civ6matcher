...
3. `bot/settings.json` でロール設定を行います。

   ```json
   {
     "channel_role_id_map": {
       "1280186565334929570": 1280187004092547112,
       "1280186823016054867": 1280187004092547112
     },
     "channel_role_map": {
     ...
     "role_weights": {
       "1280186048395218995": 4,
       "1280186025762750583": 3,
       "1280185996184522927": 2,
       "default": 1
     }
   }
   ```

   - `channel_role_id_map`: チャンネルIDとメンション対象ロールIDの対応（優先）
   - `channel_role_map`: チャンネル名に含まれるキーワードと、メンション対象ロールIDの対応（フォールバック）
   - `role_weights`: チーム分けに使用するロールIDと重み（数値が大きいほど強い）
...
- `/bo [募集タイトル]`
  - チャンネルIDに応じて対応するロールをメンションし、埋め込みメッセージを生成します。
  - メンション対象の決定は以下の優先順位で行われます：
    1. `settings.json` の `channel_role_id_map` でチャンネルIDに対応するロールIDを検索
    2. 見つからない場合は `channel_role_map` でチャンネル名から対応するロールIDを検索
...
- 参加者のロールに応じて重み付けが行われます（重みは `bot/settings.json` の `role_weights` で設定）：
  - デフォルト設定:
    - ロールID `1280186048395218995`: 重み 4
    - ロールID `1280186025762750583`: 重み 3
    - ロールID `1280185996184522927`: 重み 2
    - その他: 重み 1
...
Bot の設定を動的に管理するコマンドです。設定ファイル（`bot/settings.json`）を物理的に編集することなく、メモリ内で設定を一時的に変更できます。

#### アクション

- `/settings action:show`
  - 現在の設定内容を JSON 形式で表示します。

- `/settings action:set value:<JSON値>`
  - settings.json全体を上書きします。
  - 例: `/settings action:set value:{"channel_role_map": {"エンジョイ卓": 1280187004092547112}, "role_weights": {"1280186048395218995": 4, "default": 1}}`
  - valueには完全なJSON形式の設定オブジェクトを指定してください。

- `/settings action:clear`
  - すべてのオーバーライドをクリアし、`bot/settings.json` の内容に戻します。

- `/settings action:reload`
  - `bot/settings.json` から設定を再読み込みします。

**注意**: `/settings` コマンドによる変更はメモリ内のみで行われ、Bot を再起動すると元の設定ファイルの内容に戻ります。
...
- チャンネル名とロール ID の対応、重み付け対象ロール ID は `bot/settings.json` で管理しています。サーバー構成に合わせて編集してください。
- 設定はランタイムで `/settings` コマンドを使って一時的に変更することもできます（メモリ内のみ、再起動で元に戻ります）。
...
├── bot/
│   ├── __init__.py
│   ├── config.py          # Bot設定と環境変数の読み込み
│   ├── main.py            # エントリーポイント
│   ├── settings.json      # ロール設定（チャンネルマッピング、重み付け）
│   └── commands/
│       ├── __init__.py
│       ├── bo.py          # 募集・チーム分けコマンド
│       ├── settings.py    # 設定管理コマンド
│       └── ping.py        # 疎通確認コマンド
├── docker-compose.yml
├── dockerfile
├── README.md

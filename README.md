...
3. `bot/settings.json` でロール設定を行います。
   ...
   - `channel_role_map`: チャンネル名に含まれるキーワードと、メンション対象ロールIDの対応
...
- `/settings action:set key:<キー> value:<JSON値>`
  - 指定したキーの設定を上書きします。
  - 例: `/settings action:set key:channel_role_map value:{"テスト卓": 123456789}`
  - 使用可能なキー:
    - `channel_role_map`: チャンネル名とロールIDのマッピング
    - `role_weights`: ロールIDと重みのマッピング
